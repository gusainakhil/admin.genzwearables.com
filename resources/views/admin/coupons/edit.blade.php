...
<div class="w-full">
    <div class="rounded-[28px] border border-rose-200/60 bg-white shadow-sm">
        <div class="border-b border-rose-100/80 px-6 py-5">
            ...
            <h3 class="mt-1 text-xl font-semibold text-stone-900">Edit Coupon</h3>
        </div>
        <div class="p-6">
            <form action="{{ route('admin.coupons.update', $coupon) }}" method="POST" class="space-y-5">
                ...
                <div>
                    ...
                    <input type="number" min="1" name="user_usage_limit" value="{{ old('user_usage_limit', $coupon->user_usage_limit) }}"
                        class="w-full rounded-2xl border border-rose-200/60 bg-white px-4 py-3 text-sm text-stone-800 shadow-sm focus:border-rose-400 focus:outline-none"
